...
<div
	...
	in:fade
	class={`rep-card hover:shadow-outline focus:shadow-outline border flex cursor-pointer flex-col content-center overflow-hidden rounded border-2 shadow-lg border-${party}`}
>
	<RepImage {fullName} {id} hideOnMobile={true} />
	<div class="px-6 py-4">
		<div class="mb-2 text-xl font-bold">{fullName}</div>
	...
	<div class="px-6 py-4">
		<span
			class={`inline-block bg-${party} rounded-full px-3 py-1 text-sm font-semibold text-${party === 'libertarian' ? 'gray-700' : 'white'} mr-2`}
			>{term.party}</span
		>
	</div>
</div>
