<script>
	...
	const [stateValidity, stateValidate] = createFieldValidator(
		requiredValidator(),
		stateCodeValidator()
	);
	const [districtValidity, districtValidate] = createFieldValidator(
		requiredValidator(),
		districtValidator()
	);
...
<form>
	<label class="mb-2 block text-sm font-bold text-gray-700" for="state">State Code</label>
	<input
		id="state"
	...
	{#if !editing.state && $stateValidity.dirty && !$stateValidity.valid}
		<span class="text-md italic text-republican">
			{$stateValidity.message}
		</span>
	{/if}

	<label class="mb-2 block text-sm font-bold text-gray-700" for="district">District Code</label>
	<input
		id="district"
		type="text"
		placeholder="11"
		...
		bind:value={district}
		class="border focus:shadow-outline mb-3 w-full appearance-none rounded border-red-500 px-3 py-2 leading-tight text-gray-700 shadow focus:outline-none"
	...
	<button
		...
		class="rounded bg-democrat px-4 py-2 font-bold text-white hover:bg-blue-700"
